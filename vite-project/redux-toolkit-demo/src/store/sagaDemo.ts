 ...
 * 特点：可以同时运行多个 saga 实例
 */
function* takeEveryWorker(
  action: ReturnType<typeof demoActions.triggerTakeEvery>
) {
  try {
 ...
 * 特点：只保留最新的 saga 实例
 */
function* takeLatestWorker(
  action: ReturnType<typeof demoActions.triggerTakeLatest>
) {
  try {
